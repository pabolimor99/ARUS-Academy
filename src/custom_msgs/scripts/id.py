...
def publish_cone_ids(cones, publisher):
    ...
    for cone in cones:
        ...
        marker.text = str(cone['id'])
        marker_array.markers.append(marker)
    publisher.publish(marker_array)

def callback(msg):
    global cones
    cones = [{'id': cone.id, 'position': (cone.position.x, cone.position.y), 'color': cone.color} for cone in msg.cones]
    publish_cones(cones, cone_publisher)
    publish_cone_ids(cones, id_publisher)
    procesa_ruta()

def listener():
    global route_publisher, triangles_publisher, cone_publisher, id_publisher
    ...
    id_publisher = rospy.Publisher('visualization_cone_ids', MarkerArray, queue_size=10)
